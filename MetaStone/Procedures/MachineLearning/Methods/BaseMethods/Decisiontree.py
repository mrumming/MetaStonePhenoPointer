...
class DecisionTree(ClassifierModel):
    ...
    def train(self, dataframe, metadata_label, *args, **kwargs):
        ...
        if not kwargs:
            kwargs = {"criterion": "gini", "splitter": "best", "presort": False}
        ...
        if len(metadata_labels) == 1:
            ...
            print("Moere labels")
            self.classifier.fit(dataframe[pfams].values, dataframe[metadata_labels].values)
    ...
    def cross_validate_stratifiedkfold(self, dataframe, metadata_label, multilabel=False, binary=False, n_folds=3,
        ...
        for i, (train, test) in enumerate(skf):
            ...
            if completeness == 1.:
                print("COMPLETE")
                preds = self.classifier.predict(dataframe.iloc[test][self.pfams].values, **kwargs)
            else:
                print("PARTIAL")
...
ClassifierModel.register("dt", DecisionTree)
