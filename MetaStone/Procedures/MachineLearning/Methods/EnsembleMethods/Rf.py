        ...
        for i, (train, test) in enumerate(skf):
            ...
            if completeness == 1.:
                ...
                for shap in range(df_cp.shape[0]):
                    try:
                        for choi in np.random.choice(df_cp[shap].nonzero()[0], int(np.count_nonzero(df_cp[shap]) * (1 - completeness)), replace=False):
                            df_cp[shap][choi] = .0
                    except:
            ...
            if multilabel:
                ...
                #                                                  average="binary", pos_label="Sporulating")
...
ClassifierModel.register("rf", RF)
