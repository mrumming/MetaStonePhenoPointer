...
class RFPCA(ClassifierModel):
    ...
    def train(self, dataframe, metadata_label, *args, **kwargs):
        ...
        if len(self.labels) == 1:
            print("One label")
            print(self.labels)
            self.classifier.fit(self.pca.fit_transform(dataframe[self.pfams].values), dataframe[metadata_label].values)

        else:
            print("Moere labels")
            self.classifier.fit(self.pca.fit_transform(dataframe[self.pfams].values), dataframe[self.labels].values)
    ...
    def classify(self, dataframe, metadata_label, *args, **kwargs):
        ...
        preds = self.classifier.predict(self.pca.transform(dataframe[self.pfams].values))
    ...
    def cross_validate_stratifiedkfold(self, dataframe, metadata_label, multilabel=False, binary=False, n_folds=3.,
        ...
        for i, (train, test) in enumerate(skf):
            ...
            if completeness == 1.:
                print("COMPLETE")
                preds = self.classifier.predict(self.pca.transform(dataframe.iloc[test][self.pfams].values))
            else:
                print("PARTIAL")
                ...
                for shap in range(df_cp.shape[0]):
                    for choi in np.random.choice(df_cp[shap].nonzero()[0],
                                                 ...
                                                 replace=False):
                        df_cp[shap][choi] = .0
                preds = self.classifier.predict(self.pca.transform(df_cp))
    ...
    def save(self, filename):
        ...
        if filename.endswith(".json"):
            ...
            IO.write_classifier_pickle(self.pca, filename[:-4] + ".pkld.pca")
        else:
            IO.write_classifier_pickle(self.classifier, filename)
            IO.write_classifier_pickle(self.pca, filename + ".pca")

    def load(self, filename):
        """ Load from file """
        self.classifier = IO.read_pickled_classifier(filename)
        self.pca = IO.read_pickled_classifier(filename + ".pca")
...
ClassifierModel.register("rfpca", RFPCA)
